...
while cv.waitKey(1) < 0:
    ...
    #x = [x for x, v in points.items()]
    try:
        #if points[1] == None or points[3] == None or points[6] == None :
         #   pass
        ...
        if points[6][1] >= points[8][1]:
            ...
            # cvimg = today.strftime('%Y-%m-%D %H:%M') + '.png'
            # cv.imwrite(cvimg, frame)
            # img = cv.imread(cvimg)
        ...
        # elif points[3][0] <= points[8][0]:
            # print('falling')
        # elif points[6][0] >= points[8][0]:
        #     print('Fall-Detection!')
        # else:
            # print('Walking')
            #cv.putText(frame, 'Walking!', (10, 30),cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv.LINE_AA)
